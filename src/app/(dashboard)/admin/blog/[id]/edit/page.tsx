...
import { ArrowLeft, Sparkles, Save, Eye, Search, Loader } from "lucide-react";
...
export default function EditBlogPostPage() {
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      alert("AI generation failed. Check OpenAI API key.");
    } finally {
      setAiLoading(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Changes save ho jaenge, published status maintain rahega</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="gap-2" onClick={() => setShowAiPanel(!showAiPanel)}>
            <Sparkles className="w-4 h-4 text-purple-500" /> AI Rewrite
          </Button>
          <Button size="sm" onClick={save} disabled={saving} className="gap-2 bg-orange-500 hover:bg-orange-600">
      ...
      {showAiPanel && (
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-purple-900">AI Blog Rewrite</h3>
          </div>
          <div className="flex gap-3 flex-col md:flex-row">
            <input value={aiKeyword} onChange={(e) => setAiKeyword(e.target.value)}
              placeholder="New topic ya current topic improve karna hai?"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-5">
          <div>
            ...
            <input value={form.title} onChange={(e) => update("title", e.target.value)}
              className="w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 font-medium"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Excerpt</label>
            <textarea value={form.excerpt} onChange={(e) => update("excerpt", e.target.value)}
              rows={2} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 resize-none"
            />
          </div>
          <div>
        ...
        <div className="space-y-5">
          <div className="bg-white border rounded-xl p-4 space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100">
              <div>
                ...
                <p className="text-xs text-gray-400">Public blog par dikhega</p>
              </div>
              <button onClick={() => update("published", !form.published)}
                className={`relative w-10 h-5 rounded-full transition-colors ${form.published ? "bg-green-500" : "bg-gray-200"}`}>
            ...
            <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-100">
              <div>
                ...
                <p className="text-xs text-gray-400">Homepage highlight</p>
              </div>
          ...
          <div className="bg-white border rounded-xl p-4 space-y-4">
            <h3 className="font-semibold text-gray-900 text-sm flex items-center gap-2"><Search className="w-4 h-4" />SEO</h3>
            <div>
